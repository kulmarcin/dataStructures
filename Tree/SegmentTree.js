function build(index, from, to) {
  ...
  if (isLeafNode(to)) {
    const leafVal = this.containsObjects
      ? this.array[from][this.propertyKey]
      : this.array[from];
    ...
    this.heap[index].sum =
      this.heap[2 * index].sum + this.heap[2 * index + 1].sum;
    this.heap[index].min = Math.min(
      this.heap[2 * index].min,
      this.heap[2 * index + 1].min
    );
    this.heap[index].max = Math.max(
      this.heap[2 * index].max,
      this.heap[2 * index + 1].max
    );
  }
}
...
const t = new SegmentTree([1,2,3,4,5])
console.log(t)